...
public sealed class BillingSessionConfiguration : IEntityTypeConfiguration<BillingSession>
{
    public void Configure(EntityTypeBuilder<BillingSession> builder)
    {
        ...
        builder.Property(bs => bs.Status).HasConversion<string>().HasMaxLength(20).HasDefaultValue("Pending");
        ...
        builder.HasOne(bs => bs.User).WithMany(u => u.BillingSessions).HasForeignKey(bs => bs.UserId).OnDelete(DeleteBehavior.Cascade);
