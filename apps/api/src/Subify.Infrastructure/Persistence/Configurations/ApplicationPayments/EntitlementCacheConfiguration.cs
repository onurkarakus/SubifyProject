...
public sealed class EntitlementCacheConfiguration : IEntityTypeConfiguration<EntitlementCache>
{
    public void Configure(EntityTypeBuilder<EntitlementCache> builder)
    {
        ...
        builder.Property(ec => ec.Status).HasConversion<string>().HasMaxLength(20).HasDefaultValue("Active");
        ...
        builder.HasOne(ec => ec.User).WithMany(u => u.Entitlements).HasForeignKey(ec => ec.UserId).OnDelete(DeleteBehavior.Cascade);
