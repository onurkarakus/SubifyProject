...
public sealed class SubscriptionPaymentRecordConfiguration : IEntityTypeConfiguration<SubscriptionPaymentRecord>
{
    public void Configure(EntityTypeBuilder<SubscriptionPaymentRecord> builder)
    {
        ...
        builder.Property(pr => pr.Status).HasConversion<string>().HasMaxLength(20).HasDefaultValue("Paid");
        ...
        builder.HasOne(pr => pr.Subscription).WithMany(s => s.PaymentRecords).HasForeignKey(pr => pr.SubscriptionId).OnDelete(DeleteBehavior.Cascade);
        builder.HasOne(pr => pr.User).WithMany().HasForeignKey(pr => pr.UserId).OnDelete(DeleteBehavior.NoAction);
